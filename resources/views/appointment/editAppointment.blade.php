...
<body class="bg-blue-950">
    <div class="relative m-5 overflow-x-auto mx-14">
        <div class="text-white text-xl font-bold mb-3">{{ __('text.editApm') }}</div>

        <form action="/appointment/{{ $apmInfo->id }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-6 ">
                <label for="a_drName" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                ...
                <select name="a_drName" id="a_drName"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    <option selected>{{ $apmInfo->a_drName }}</option>
                    @foreach ($docList as $doc)
                    <option value="{{ $doc->d_drName }}">{{ $doc->d_drName }}</option>
                @endforeach 
                </select>
            ...
            <div class="mb-6">
                ...
                <input type="datetime-local" id="a_dateTime" name="a_dateTime"
                    class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light"
                    required value="{{ $apmInfo->a_dateTime }}">
            </div>
            <button type="submit"
                class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                {{ __('text.editApmBtn') }}</button>
        </form>
        <a href="/appointment">
            <button
