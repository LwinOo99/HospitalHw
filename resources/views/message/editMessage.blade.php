...
<body class="bg-blue-950">
    <div class="relative m-5 overflow-x-auto mx-14">
        <div class="text-white text-xl font-bold mb-3">{{ __("text.editMsg") }}</div>

        <form action="/message/{{ $msgInfo->id }}" method="POST">
            @csrf
            @method("PUT")
            <div class="mb-6 ">
                ...
                <input type="text" id="newMsg" name="m_message"
                    ...
                    value="{{ $msgInfo->m_message }}" >
            </div>
            <div class="mb-6">
                <input type="radio" name="m_vip" id="nonvip" value="0" @checked($msgInfo->m_vip== 0)>
                <label for="active" class=" mb-2 text-sm font-medium text-gray-900 dark:text-white">Standart</label>

                <input type="radio" name="m_vip" id="vip" value="1" @checked($msgInfo->m_vip== 1)>
